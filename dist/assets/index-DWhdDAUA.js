(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const s of r.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function t(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(n){if(n.ep)return;n.ep=!0;const r=t(n);fetch(n.href,r)}})();const ie="modulepreload",re=function(l){return"/"+l},W={},O=function(e,t,i){let n=Promise.resolve();if(t&&t.length>0){let s=function(o){return Promise.all(o.map(d=>Promise.resolve(d).then(f=>({status:"fulfilled",value:f}),f=>({status:"rejected",reason:f}))))};document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),u=a?.nonce||a?.getAttribute("nonce");n=s(t.map(o=>{if(o=re(o),o in W)return;W[o]=!0;const d=o.endsWith(".css"),f=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${o}"]${f}`))return;const h=document.createElement("link");if(h.rel=d?"stylesheet":ie,d||(h.as="script"),h.crossOrigin="",h.href=o,u&&h.setAttribute("nonce",u),document.head.appendChild(h),d)return new Promise((g,m)=>{h.addEventListener("load",g),h.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${o}`)))})}))}function r(s){const a=new Event("vite:preloadError",{cancelable:!0});if(a.payload=s,window.dispatchEvent(a),!a.defaultPrevented)throw s}return n.then(s=>{for(const a of s||[])a.status==="rejected"&&r(a.reason);return e().catch(r)})};async function se(l){if(!navigator.gpu)throw new Error("WebGPU not supported");const e=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(!e)throw new Error("No WebGPU adapter found");const t={},i=(o,d)=>{const f=e.limits[o];t[o]=Math.min(d,f)};i("maxStorageBufferBindingSize",256*1024*1024),i("maxBufferSize",256*1024*1024),i("maxComputeWorkgroupSizeX",256),i("maxComputeInvocationsPerWorkgroup",256),i("maxStorageBuffersPerShaderStage",8);const n=[],r=e.features.has("timestamp-query");r&&n.push("timestamp-query"),e.features.has("subgroups")&&n.push("subgroups");const s=await e.requestDevice({requiredLimits:t,requiredFeatures:n});s.lost.then(o=>{console.error("WebGPU device lost:",o.message),o.reason!=="destroyed"&&console.warn("Attempting recovery would go here")});const a=l.getContext("webgpu");if(!a)throw new Error("Failed to get WebGPU canvas context");const u=navigator.gpu.getPreferredCanvasFormat();return a.configure({device:s,format:u,alphaMode:"premultiplied"}),{device:s,context:a,format:u,canvas:l,supportsTimestampQuery:r,features:s.features}}class ae{device;available=[];inFlight=0;constructor(e){this.device=e}acquire(e){let t=-1,i=1/0;for(let s=0;s<this.available.length;s++){const a=this.available[s].size;a>=e&&a<i&&(t=s,i=a)}if(t>=0){const s=this.available.splice(t,1)[0];return this.inFlight++,s.buffer}const n=Math.max(oe(e,4096),4096),r=this.device.createBuffer({label:`staging-ring-${n}`,size:n,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST});return this.inFlight++,r}release(e,t){this.inFlight--,this.available.length<6?this.available.push({buffer:e,size:t}):e.destroy()}destroy(){for(const e of this.available)e.buffer.destroy();this.available.length=0}}function oe(l,e){return Math.ceil(l/e)*e}class ue{buffers=new Map;device;stagingRing;constructor(e){this.device=e,this.stagingRing=new ae(e)}createBuffer(e,t,i,n){this.destroyBuffer(e);const r=this.device.createBuffer({label:n??e,size:Math.max(t,4),usage:i});return this.buffers.set(e,r),r}uploadData(e,t,i=0){const n=this.buffers.get(e);if(!n)throw new Error(`Buffer "${e}" not found`);ArrayBuffer.isView(t)?this.device.queue.writeBuffer(n,i,t.buffer,t.byteOffset,t.byteLength):this.device.queue.writeBuffer(n,i,t)}getBuffer(e){const t=this.buffers.get(e);if(!t)throw new Error(`Buffer "${e}" not found`);return t}hasBuffer(e){return this.buffers.has(e)}destroyBuffer(e){const t=this.buffers.get(e);t&&(t.destroy(),this.buffers.delete(e))}destroyAll(){for(const e of this.buffers.values())e.destroy();this.buffers.clear(),this.stagingRing.destroy()}async readBuffer(e,t){const i=this.getBuffer(e),n=this.stagingRing.acquire(t),r=this.device.createCommandEncoder();r.copyBufferToBuffer(i,0,n,0,t),this.device.queue.submit([r.finish()]),await n.mapAsync(GPUMapMode.READ);const s=new Float32Array(n.getMappedRange(0,t).slice(0));return n.unmap(),this.stagingRing.release(n,n.size),s}}const L=64,le=60;class de{enabled;querySet=null;resolveBuffer=null;readbackBuffer=null;queryIndex=0;stages=[];mapping=!1;latestTimings=null;frameCount=0;constructor(e,t){this.enabled=t,this.enabled&&(this.querySet=e.createQuerySet({type:"timestamp",count:L}),this.resolveBuffer=e.createBuffer({label:"profiler-resolve",size:L*8,usage:GPUBufferUsage.QUERY_RESOLVE|GPUBufferUsage.COPY_SRC}),this.readbackBuffer=e.createBuffer({label:"profiler-readback",size:L*8,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}))}beginFrame(){this.enabled&&(this.queryIndex=0,this.stages.length=0)}timestampWrites(e){if(!this.enabled||!this.querySet||this.queryIndex+2>L)return;const t=this.queryIndex,i=this.queryIndex+1;return this.queryIndex+=2,this.stages.push(e),{querySet:this.querySet,beginningOfPassWriteIndex:t,endOfPassWriteIndex:i}}resolve(e){!this.enabled||!this.querySet||!this.resolveBuffer||!this.readbackBuffer||this.queryIndex!==0&&(e.resolveQuerySet(this.querySet,0,this.queryIndex,this.resolveBuffer,0),e.copyBufferToBuffer(this.resolveBuffer,0,this.readbackBuffer,0,this.queryIndex*8))}async readback(){if(!this.enabled||!this.readbackBuffer||this.mapping||this.queryIndex===0)return null;const e=this.stages.length,t=this.stages.slice();this.mapping=!0;try{await this.readbackBuffer.mapAsync(GPUMapMode.READ,0,e*2*8);const i=new BigUint64Array(this.readbackBuffer.getMappedRange(0,e*2*8)),n=new Map;for(let s=0;s<e;s++){const a=i[s*2],u=i[s*2+1],o=Number(u-a),d=t[s];n.set(d,(n.get(d)??0)+o)}this.readbackBuffer.unmap();const r=[];for(const[s,a]of n)r.push({stage:s,ms:a/1e6});if(this.latestTimings=r,this.frameCount++,this.frameCount%le===0){const s=r.map(a=>`${a.stage}: ${a.ms.toFixed(3)}ms`);console.log(`[GPUProfiler] ${s.join(" | ")}`)}return r}catch{return null}finally{this.mapping=!1}}getLatestTimings(){return this.latestTimings}destroy(){this.querySet?.destroy(),this.resolveBuffer?.destroy(),this.readbackBuffer?.destroy(),this.querySet=null,this.resolveBuffer=null,this.readbackBuffer=null}}function fe(l,e,t,i,n,r){const s=new Float32Array(16),a=e-l,u=i-t,o=r-n;return s[0]=2/a,s[5]=2/u,s[10]=-2/o,s[12]=-(e+l)/a,s[13]=-(i+t)/u,s[14]=-0/o,s[15]=1,s}class he{center=[0,0];zoom=1;minZoom=5e-4;maxZoom=50;width=1;height=1;projectionDirty=!0;cachedProjection=new Float32Array(16);version=0;resize(e,t){this.width=e,this.height=t,this.projectionDirty=!0,this.version++}getProjection(){if(this.projectionDirty){const e=this.width/2/this.zoom,t=this.height/2/this.zoom;this.cachedProjection=fe(this.center[0]-e,this.center[0]+e,this.center[1]-t,this.center[1]+t,-1,1),this.projectionDirty=!1}return this.cachedProjection}pan(e,t){this.center[0]-=e/this.zoom,this.center[1]+=t/this.zoom,this.projectionDirty=!0,this.version++}zoomAt(e,t,i){const n=this.screenToWorld(e,t);this.zoom=Math.max(this.minZoom,Math.min(this.maxZoom,this.zoom*i));const r=this.screenToWorld(e,t);this.center[0]+=n[0]-r[0],this.center[1]+=n[1]-r[1],this.projectionDirty=!0,this.version++}screenToWorld(e,t){const i=this.center[0]+(e-this.width/2)/this.zoom,n=this.center[1]-(t-this.height/2)/this.zoom;return[i,n]}worldToScreen(e,t){const i=(e-this.center[0])*this.zoom+this.width/2,n=-(t-this.center[1])*this.zoom+this.height/2;return[i,n]}fitBounds(e,t,i,n,r=.1){const a=Math.max(i-e,100),u=Math.max(n-t,100);this.center=[(e+i)/2,(t+n)/2];const o=this.width/(a*(1+r)),d=this.height/(u*(1+r));this.zoom=Math.max(this.minZoom,Math.min(this.maxZoom,Math.min(o,d))),this.projectionDirty=!0,this.version++}getViewportWidth(){return this.width}getViewportHeight(){return this.height}invalidate(){this.projectionDirty=!0}}const G=[[.4,.761,.647,1],[.988,.553,.384,1],[.553,.627,.796,1],[.906,.541,.765,1],[.651,.847,.329,1],[1,.851,.184,1],[.898,.769,.58,1],[.702,.702,.702,1],[.471,.808,.922,1],[.859,.439,.576,1],[.58,.863,.541,1],[.776,.569,.894,1],[.929,.682,.38,1],[.404,.694,.82,1],[.816,.78,.369,1],[.659,.471,.71,1]];function ce(l){const e=new Float32Array(G.length*4);for(let t=0;t<G.length;t++)e[t*4+0]=G[t][0],e[t*4+1]=G[t][1],e[t*4+2]=G[t][2],e[t*4+3]=G[t][3];return e}function q(l){return G[l%G.length]}function k(l){return l.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}class ge{el;constructor(e){this.el=document.createElement("div"),this.el.className="hg-tooltip",this.el.style.cssText=`
      position: absolute;
      display: none;
      pointer-events: none;
      z-index: 100;
    ...
    `,e.appendChild(this.el)}show(e,t,i,n){const r=n.map(k),s=r.length<=5?r.join(", "):r.slice(0,3).join(", ")+`, +${n.length-3} more`;this.el.innerHTML=`<div style="font-weight:600;margin-bottom:2px">${k(i)}</div><div style="color:#666680">${s}</div>`,this.el.style.display="block",this.position(e,t)}showNode(e,t,i,n){const r=n.map(k),s=r.length===0?'<span style="color:#999">no edges</span>':r.length<=5?r.join(", "):r.slice(0,4).join(", ")+`, +${n.length-4} more`;this.el.innerHTML=`<div style="font-weight:600;margin-bottom:2px">${k(i)}</div><div style="color:#666680">${s}</div>`,this.el.style.display="block",this.position(e,t)}hide(){this.el.style.display="none"}position(e,t){const i=this.el.parentElement,n=i.clientWidth,r=i.clientHeight,s=this.el.offsetWidth,a=this.el.offsetHeight;let u=e+12,o=t+12;u+s>n&&(u=e-s-8),o+a>r&&(o=t-a-8),this.el.style.left=`${u}px`,this.el.style.top=`${o}px`}dispose(){this.el.remove()}}function pe(){return{repulsionStrength:-300,attractionStrength:.03,linkDistance:50,centerStrength:.015,velocityDecay:.6,energy:1,idleEnergy:.02,coolingRate:.0228,stopThreshold:.001,theta:.9,running:!0}}function me(){return{nodeBaseSize:10,edgeOpacity:.15,hullAlpha:.25,hullOutline:!1,hullMargin:3,hullSmoothing:4,hullMode:"metaball",hullMetaballThreshold:.5,nodeDarkMode:!0,backgroundColor:[.97,.97,.98,1]}}const be=`// Node rendering shader — generates quads from point data
...
`,ye=`// GPU Radix Sort — single pass for one 8-bit digit
...
`,_e=`// GPU Radix Sort — subgroup-accelerated histogram variant
...
`,xe=`// Bottom-up center-of-mass computation for quadtree internal nodes
//
...
`,we=`// Barnes-Hut repulsion force computation
//
...
`,Ue=`// Link spring force (attraction) for hyperedges
//
...
`,Ge=`// Centering force — prevents graph from drifting
...
`,Ce=`// Velocity Verlet integration
...
`,K=12,$=K*4;class ze{gpu;buffers;camera;pipeline;bindGroupLayout;bindGroup=null;cameraBuffer;paramsBuffer;instanceCapacity=0;mstCapacity=0;instanceCount=0;lastCameraVersion=-1;lastEdges=[];lastSigma=5;lastThreshold=.5;lastPositions=null;paramsArray=new Float32Array(4);constructor(e,t,i){this.gpu=e,this.buffers=t,this.camera=i;const{device:n,format:r}=e,s=n.createShaderModule({label:"metaball-render-shader",code:qe});this.bindGroupLayout=n.createBindGroupLayout({label:"metaball-render-bgl",entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:3,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:4,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:5,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:6,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),this.pipeline=n.createRenderPipeline({label:"metaball-render-pipeline",layout:n.createPipelineLayout({bindGroupLayouts:[this.bindGroupLayout]}),vertex:{module:s,entryPoint:"vs_main"},fragment:{module:s,entryPoint:"fs_main",targets:[{format:r,blend:{color:{srcFactor:"src-alpha",dstFactor:"one-minus-src-alpha",operation:"add"},alpha:{srcFactor:"one",dstFactor:"one-minus-src-alpha",operation:"add"}}}]},primitive:{topology:"triangle-list"}}),this.cameraBuffer=t.createBuffer("metaball-camera",64,GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST,"metaball-camera"),this.paramsBuffer=t.createBuffer("metaball-render-params",16,GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST,"metaball-render-params")}updateInstances(e,t,i,n,r,s){this.lastEdges=t,this.lastSigma=i,this.lastThreshold=n,this.lastPositions=e;const a=[];for(const c of t)c.memberIndices.length>=2&&a.push(c);if(a.length===0){this.instanceCount=0;return}const u=i*3,o=a.length,d=new ArrayBuffer(o*$),f=new Float32Array(d),h=new Uint32Array(d),g=[];let m=0;for(const c of a){const p=c.memberIndices.map(v=>[e[v*4],e[v*4+1]]),y=j(p).map(([v,S])=>[c.memberIndices[v],c.memberIndices[S]]);g.push(y),m+=y.length}const _=new Uint32Array(Math.max(m*2,1));let B=0;for(let c=0;c<o;c++){const p=a[c],b=g[c],y=q(p.index),S=s!==null&&s.has(p.index)?r*.08:r;let E=1/0,w=1/0,D=-1/0,T=-1/0;for(const C of p.memberIndices){const U=e[C*4],R=e[C*4+1];U<E&&(E=U),R<w&&(w=R),U>D&&(D=U),R>T&&(T=R)}let A=i;for(const[C,U]of b){const R=e[C*4],J=e[C*4+1],ee=e[U*4],te=e[U*4+1],Y=ee-R,H=te-J,ne=Math.sqrt(Y*Y+H*H);A=Math.max(A,ne*.12)}const F=Math.max(u,A*3);E-=F,w-=F,D+=F,T+=F;const x=c*K;f[x+0]=E,f[x+1]=w,f[x+2]=D,f[x+3]=T,f[x+4]=y[0],f[x+5]=y[1],f[x+6]=y[2],f[x+7]=S,h[x+8]=p.index,h[x+9]=B,h[x+10]=b.length,h[x+11]=0;for(const[C,U]of b)_[B*2]=C,_[B*2+1]=U,B++}this.instanceCount=o;const P=o*$;P>this.instanceCapacity&&(this.instanceCapacity=P*2,this.buffers.destroyBuffer("metaball-instances"),this.buffers.createBuffer("metaball-instances",this.instanceCapacity,GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST,"metaball-instances"),this.bindGroup=null),this.buffers.uploadData("metaball-instances",new Uint8Array(d));const M=Math.max(_.byteLength,4);M>this.mstCapacity&&(this.mstCapacity=Math.max(M*2,16),this.buffers.destroyBuffer("metaball-mst"),this.buffers.createBuffer("metaball-mst",this.mstCapacity,GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST,"metaball-mst"),this.bindGroup=null),this.buffers.uploadData("metaball-mst",_),this.paramsArray[0]=i,this.paramsArray[1]=n,this.paramsArray[2]=n*.15,this.paramsArray[3]=0,this.gpu.device.queue.writeBuffer(this.paramsBuffer,0,this.paramsArray),this.bindGroup||this.rebuildBindGroup()}rebuildBindGroup(){!this.buffers.hasBuffer("node-positions")||!this.buffers.hasBuffer("he-offsets")||!this.buffers.hasBuffer("he-members")||!this.buffers.hasBuffer("metaball-instances")||!this.buffers.hasBuffer("metaball-mst")||(this.bindGroup=this.gpu.device.createBindGroup({label:"metaball-render-bind-group",layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.cameraBuffer}},{binding:1,resource:{buffer:this.buffers.getBuffer("node-positions")}},{binding:2,resource:{buffer:this.buffers.getBuffer("he-offsets")}},{binding:3,resource:{buffer:this.buffers.getBuffer("he-members")}},{binding:4,resource:{buffer:this.buffers.getBuffer("metaball-instances")}},{binding:5,resource:{buffer:this.buffers.getBuffer("metaball-mst")}},{binding:6,resource:{buffer:this.paramsBuffer}}]}))}render(e){this.instanceCount===0||!this.bindGroup||(this.camera.version!==this.lastCameraVersion&&(this.lastCameraVersion=this.camera.version,this.gpu.device.queue.writeBuffer(this.cameraBuffer,0,this.camera.getProjection())),e.setPipeline(this.pipeline),e.setBindGroup(0,this.bindGroup),e.draw(6,this.instanceCount))}hitTest(e,t){const i=this.lastPositions;if(!i||this.lastEdges.length===0)return null;const n=this.lastSigma,r=this.lastThreshold,s=1/(2*n*n),a=9*n*n;for(let u=this.lastEdges.length-1;u>=0;u--){const o=this.lastEdges[u];if(o.memberIndices.length<2)continue;const d=o.memberIndices.map(c=>[i[c*4],i[c*4+1]]),f=j(d);let h=1/0,g=1/0,m=-1/0,_=-1/0;for(const c of o.memberIndices){const p=i[c*4],b=i[c*4+1];p<h&&(h=p),b<g&&(g=b),p>m&&(m=p),b>_&&(_=b)}let B=n;for(const[c,p]of f){const b=d[p][0]-d[c][0],y=d[p][1]-d[c][1],v=Math.sqrt(b*b+y*y);B=Math.max(B,v*.12)}const P=Math.max(n*3,B*3);if(e<h-P||e>m+P||t<g-P||t>_+P)continue;let M=0;for(const c of o.memberIndices){const p=i[c*4],b=i[c*4+1],y=e-p,v=t-b,S=y*y+v*v;S<a&&(M+=Math.exp(-S*s))}for(const[c,p]of f){const b=d[c][0],y=d[c][1],v=d[p][0],S=d[p][1],E=Math.sqrt((v-b)**2+(S-y)**2),w=Math.max(n,E*.12),D=1/(2*w*w),T=9*w*w,A=Oe(e,t,b,y,v,S);A<T&&(M+=Math.exp(-A*D))}if(M>=r)return o.index}return null}invalidateBindGroup(){this.bindGroup=null}destroy(){this.buffers.destroyBuffer("metaball-instances"),this.buffers.destroyBuffer("metaball-mst"),this.buffers.destroyBuffer("metaball-camera"),this.buffers.destroyBuffer("metaball-render-params"),this.instanceCount=0,this.instanceCapacity=0,this.mstCapacity=0}}const Ne=`// Hull rendering shader — semi-transparent convex hull polygons
