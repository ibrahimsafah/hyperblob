...
export class MetaballRenderer {
    ...
    paramsArray = new Float32Array(4);
    constructor(gpu, buffers, camera) {
        ...
        this.cameraBuffer = buffers.createBuffer('metaball-camera', 64, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 'metaball-camera');
        this.paramsBuffer = buffers.createBuffer('metaball-render-params', 16, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 'metaball-render-params');
    }
    /**
     ...
     */
    updateInstances(positions, edges, sigma, threshold, alpha, dimmedEdges) {
        ...
        if (instanceBytes > this.instanceCapacity) {
            ...
            this.buffers.createBuffer('metaball-instances', this.instanceCapacity, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST, 'metaball-instances');
        ...
        if (mstBytes > this.mstCapacity) {
            ...
            this.buffers.createBuffer('metaball-mst', this.mstCapacity, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST, 'metaball-mst');
            this.bindGroup = null; // force rebind
        }
    ...
    render(renderPass) {
        if (this.instanceCount === 0 || !this.bindGroup)
            return;
    ...
    hitTest(worldX, worldY) {
        ...
        if (!positions || this.lastEdges.length === 0)
            return null;
        ...
        for (let e = this.lastEdges.length - 1; e >= 0; e--) {
            ...
            if (edge.memberIndices.length < 2)
                continue;
