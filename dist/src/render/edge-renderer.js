...
export class EdgeRenderer {
    ...
    bindGroup = null;
    cameraBuffer = null;
    edgeParamsBuffer = null;
    // Total number of line segments (each = 2 vertices)
    totalLineSegments = 0;
    lastCameraVersion = -1;
    edgeParamsArray = new Float32Array(4);
    edgeCount = 0;
    ...
    initPipeline() {
        ...
        this.cameraBuffer = this.buffers.createBuffer('edge-camera-uniform', 64, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 'edge-camera-uniform');
        // Edge rendering params uniform (opacity + padding)
        this.edgeParamsBuffer = this.buffers.createBuffer('edge-params-uniform', 16, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 'edge-params-uniform');
    }
    /**
    ...
    setData(data) {
        ...
        this.buffers.createBuffer('edge-draw-indices', drawData.byteLength, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST, 'edge-draw-indices');
        ...
        this.buffers.createBuffer('edge-flags', flagsSize, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST, 'edge-flags');
     ...
     */
    setVisibleEdges(data, visibleEdges) {
        let totalSegments = 0;
        for (const he of data.hyperedges) {
    ...
    setDimmedEdges(dimmedSet) {
        if (!this.buffers.hasBuffer('edge-flags') || this.edgeCount === 0)
            return;
        const flags = new Uint32Array(this.edgeCount);
        if (dimmedSet) {
            for (const idx of dimmedSet) {
                if (idx < this.edgeCount)
                    flags[idx] = 1;
            }
        }
    ...
    recreateBindGroup() {
        if (!this.pipeline || !this.cameraBuffer || !this.edgeParamsBuffer)
            return;
        if (!this.buffers.hasBuffer('node-positions'))
            return;
        if (!this.buffers.hasBuffer('edge-draw-indices'))
            return;
        if (!this.buffers.hasBuffer('he-offsets'))
            return;
        if (!this.buffers.hasBuffer('he-members'))
            return;
        if (!this.buffers.hasBuffer('edge-flags'))
            return;
    ...
    render(renderPass, renderParams) {
        if (!this.pipeline || !this.bindGroup || !this.cameraBuffer || !this.edgeParamsBuffer)
            return;
        if (this.totalLineSegments === 0)
            return;
