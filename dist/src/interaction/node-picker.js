...
export class NodePicker {
    ...
    ensureTextures(width, height) {
        if (this.width === width && this.height === height && this.pickTexture)
            return;
    ...
    renderPickBuffer(encoder, positions, nodeCount, camera, viewport) {
        if (!this.pipeline)
            return;
        const [width, height] = viewport;
        this.ensureTextures(width, height);
        if (!this.pickTextureView)
            return;
    ...
    async pickAt(x, y) {
        if (!this.pickTexture || !this.readBuffer)
            return null;
        // Clamp coordinates
