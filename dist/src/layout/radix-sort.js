...
export class RadixSort {
    ...
    paramsArray = new Uint32Array(4);
    // Cached bind groups for even/odd passes (ping→pong vs pong→ping)
    evenBindGroup = null;
    oddBindGroup = null;
    get hasSubgroups() { return this._hasSubgroups; }
    constructor(device, bufferManager, maxNodeCount, profiler, features = new Set()) {
        this.device = device;
    ...
    rebuildBindGroups(numWorkgroups) {
    ...
    encode(encoder, nodeCount) {
        ...
        encoder.copyBufferToBuffer(this.bufferManager.getBuffer('morton-codes'), 0, this.bufferManager.getBuffer('sort-keys-ping'), 0, nodeCount * 4);
        encoder.copyBufferToBuffer(this.bufferManager.getBuffer('sorted-indices'), 0, this.bufferManager.getBuffer('sort-vals-ping'), 0, nodeCount * 4);
        ...
        encoder.copyBufferToBuffer(this.bufferManager.getBuffer('sort-keys-ping'), 0, this.bufferManager.getBuffer('morton-codes'), 0, nodeCount * 4);
        encoder.copyBufferToBuffer(this.bufferManager.getBuffer('sort-vals-ping'), 0, this.bufferManager.getBuffer('sorted-indices'), 0, nodeCount * 4);
    }
    destroy() {
        const names = [
