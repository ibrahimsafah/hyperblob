...
export class GPUQuadtree {
    ...
    // Cached bind groups
    buildBindGroup = null;
    summarizeBindGroup = null;
    // Pre-allocated param arrays with dual views
    buildParamsArray = new Uint32Array(4);
    summarizeParamsBuf = new ArrayBuffer(16);
    summarizeParamsU32 = new Uint32Array(this.summarizeParamsBuf);
    summarizeParamsF32 = new Float32Array(this.summarizeParamsBuf);
    // Tree parameters
    treeSize = 0; // total nodes in tree
    leafOffset = 0; // first leaf index
    numLevels = 0; // number of levels in tree
    constructor(device, bufferManager, profiler) {
        this.device = device;
    ...
    ensureBuffers(nodeCount) {
        ...
        this.bufferManager.createBuffer('quadtree', treeBufSize, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC, 'quadtree');
        // Build params uniform
        this.bufferManager.createBuffer('quadtree-build-params', 16, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 'quadtree-build-params');
        // Summarize params uniform
        this.bufferManager.createBuffer('quadtree-summarize-params', 16, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 'quadtree-summarize-params');
     ...
     */
    encode(encoder, nodeCount, rootSize) {
        if (nodeCount === 0)
            return;
        ...
        this.device.queue.writeBuffer(this.bufferManager.getBuffer('quadtree-build-params'), 0, this.buildParamsArray);
        ...
        for (let level = this.numLevels - 2; level >= 0; level--) {
            ...
            this.device.queue.writeBuffer(this.bufferManager.getBuffer('quadtree-summarize-params'), 0, this.summarizeParamsBuf);
