...
export class Panel {
    ...
    build(config) {
        ...
        // Build tabs
        const simTabResult = createSimulationTab(config.simParams, config.onSimulationToggle, config.onSimulationReset, config.onSimulationConverge);
        this.disposers.push(simTabResult.dispose);
        const renderTab = createRenderingTab(config.renderParams);
        const dataTabResult = createDataTab(config.onLoadFile, config.onGenerate);
